...
if [ "${DOTFILES_TEST_SKIP_PACKAGES:-}" != "1" ]; then
  source_lock="/home/tester/dotfiles/dot_config/mise/mise.lock"
  installed_lock="/home/tester/.config/mise/mise.lock"
  if cmp -s "$source_lock" "$installed_lock"; then
    _sa_pass 'full lane: mise install preserves managed lockfile'
  else
    delta="$( { diff -u "$source_lock" "$installed_lock" || true; } | tail -20 | tr '\n' ' ')"
