...
- `mise install` may add checksums and URLs for common platforms to `mise.lock`. Refresh the source lock with `mise -C dot_config/mise lock` and commit the generated metadata; this resolves lock entries without installing tools. Some backends, such as `ubi`, add artifact checksums only after download. If the full-lane assertion finds one, copy the reported checksum into the matching platform entry, then rerun the full lane. See the [Mise lock command](https://mise.jdx.dev/cli/lock).
